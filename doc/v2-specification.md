...
### 5.4 Message 機能（寄り添い・励ましメッセージ）

AI が生成するプランには、ユーザーに寄り添い、励ますメッセージを含める。

#### 5.4.1 メッセージの構成

- **explanation**（説明）：1-2 文

  - 選んだ色やパラメータの理由を簡潔に説明
  - 音楽の特徴やユーザーの気持ちを考慮した選択理由を述べる
  - ユーザーの入力言語に合わせる（日本語の場合は「あなた」を使用、「ユーザー」は使用しない）

- **encouragement**（励まし）：2-4 文
  - ユーザーの状況に寄り添い、温かく励ますメッセージ
  - ユーザーの感情（喜怒哀楽）に合わせて、適切なトーンで応答
    - 喜び・楽しみ：共に喜び、前向きなエネルギーを認める
    - 悲しみ・寂しさ：慰めと理解を提供し、感情を肯定する
    - 怒り・イライラ：感情を認めつつ、冷静さと視点の転換を促す
    - 興奮・高揚：そのエネルギーに合わせ、情熱を応援する
    - 穏やか・平和：その瞬間を大切にし、マインドフルネスを促す
  - 単調にならないよう、トーンやアプローチを変化させる
  - ユーザーの入力言語に合わせる（日本語の場合は「あなた」を使用、「ユーザー」は使用しない）

#### 5.4.2 表示

- UI の「💭 Message」セクションに表示
- explanation と encouragement を統合して表示
- ユーザーが音楽を聴きながら、AI の考えや励ましを受け取れる

#### 5.4.3 目的

- 視覚的な表現だけでなく、言葉による寄り添いを提供
- ユーザーの気持ちに共感し、前向きな体験をサポート
- 音楽と AI プランが一体となった、より豊かな体験を実現
